...
const CONSENT_MESSAGES = {
  consent_privacy_policy: 'Необходимо ознакомиться с политикой конфиденциальности.',
  consent_pd_processing: 'Необходимо дать согласие на обработку персональных данных.',
};
document.querySelectorAll('input[type="checkbox"][name^="consent_"]').forEach((box) => {
  const message = CONSENT_MESSAGES[box.name];
  if (!message) return;
  const sync = () => box.setCustomValidity(box.checked ? '' : message);
  sync();
  box.addEventListener('change', sync);
});

// Web3Forms — тот же ключ и тот же получатель (asamadesign@proton.me), что и на
// действующем asama.site: при замене сайта заявки продолжат приходить туда же.
const WEB3FORMS_ACCESS_KEY = 'bee4429f-4c1c-48dc-9bc6-b82afc73a184';

// Квиз в Hero — переключение шагов
...
if (quizForm) {
  ...
  const showStep = (n) => {
    ...
    if (total) total.textContent = n <= 3 ? `${n} / 3` : 'Контакты';
    current = n;
  };

  const isFieldValid = (input) => (input.type === 'checkbox' ? input.checked : input.value.trim() !== '');
  ...
  const isStepValid = (n) => {
    ...
    return required.every(isFieldValid);
  };

  const reportStepInvalid = (n) => {
    const step = steps.find((s) => Number(s.dataset.step) === n);
    const invalid = Array.from(step.querySelectorAll('[required]')).find((input) => !isFieldValid(input));
    invalid?.reportValidity();
  };

  quizForm.querySelectorAll('[data-next]').forEach((btn) => {
    btn.addEventListener('click', () => {
      if (!isStepValid(current)) {
        reportStepInvalid(current);
        return;
      }
  ...
  quizForm.querySelectorAll('[data-back]').forEach((btn) => {
    btn.addEventListener('click', () => {
  ...
  quizForm.addEventListener('submit', async (e) => {
    ...
    if (quizForm.botcheck.checked) { quizStatus.textContent = 'Заявка отправлена. Мы свяжемся с вами в ближайшее время.'; quizForm.reset(); return; }
    if (!isStepValid(current)) {
      reportStepInvalid(current);
      return;
    }
    ...
    quizStatus.textContent = 'Отправляем…';

    const formData = new FormData(quizForm);
    formData.append('access_key', WEB3FORMS_ACCESS_KEY);
    formData.append('subject', 'Заявка с asama.site — квиз на главной');
    ...
    try {
      ...
      if (data.success) {
        quizStatus.textContent = 'Заявка отправлена. Мы свяжемся с вами в ближайшее время.';
        quizForm.reset();
        document.querySelectorAll('.quiz-option').forEach((label) => label.classList.remove('is-selected'));
        showStep(1);
      } else {
        quizStatus.textContent = 'Не получилось отправить. Попробуйте написать на asamadesign@proton.me.';
      }
    } catch (err) {
      quizStatus.textContent = 'Не получилось отправить. Попробуйте написать на asamadesign@proton.me.';
    } finally {
...
// Форма заявки внизу страницы — Web3Forms
...
if (contactForm) {
  contactForm.addEventListener('submit', async (e) => {
    ...
    // Спам-ловушка: боту показываем обычный успех, чтобы он не пробовал снова.
    if (contactForm.botcheck.checked) { formStatus.textContent = 'Заявка отправлена. Мы свяжемся с вами в ближайшее время.'; contactForm.reset(); return; }
    ...
    formData.append('access_key', WEB3FORMS_ACCESS_KEY);
    formData.append('subject', contactForm.dataset.subject || 'Заявка с asama.site — форма на главной');
    ...
    try {
      ...
      if (data.success) {
        ...
        formStatus.textContent = 'Не получилось отправить. Попробуйте написать на asamadesign@proton.me.';
      }
    } catch (err) {
      formStatus.textContent = 'Не получилось отправить. Попробуйте написать на asamadesign@proton.me.';
    } finally {
...
// Развёрнутая форма на странице contact.html
const fullContactForm = document.getElementById('full-contact-form');
const fullFormStatus = document.getElementById('full-form-status');

if (fullContactForm) {
  fullContactForm.addEventListener('submit', async (e) => {
    e.preventDefault();

    // Спам-ловушка: боту показываем обычный успех, чтобы он не пробовал снова.
    if (fullContactForm.botcheck.checked) { fullFormStatus.textContent = 'Заявка отправлена. Мы свяжемся с вами в ближайшее время.'; fullContactForm.reset(); return; }
    if (!fullContactForm.checkValidity()) { fullContactForm.reportValidity(); return; }

    const submitBtn = fullContactForm.querySelector('button[type="submit"]');
    submitBtn.disabled = true;
    fullFormStatus.textContent = 'Отправляем…';

    const formData = new FormData(fullContactForm);
    formData.append('access_key', WEB3FORMS_ACCESS_KEY);
    formData.append('subject', 'Заявка с asama.site — страница контактов');
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        fullFormStatus.textContent = 'Заявка отправлена. Мы свяжемся с вами в ближайшее время.';
        fullContactForm.reset();
      } else {
        fullFormStatus.textContent = 'Не получилось отправить. Попробуйте написать на asamadesign@proton.me.';
      }
    } catch (err) {
      fullFormStatus.textContent = 'Не получилось отправить. Попробуйте написать на asamadesign@proton.me.';
    } finally {
